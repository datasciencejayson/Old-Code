...
# -*- coding: utf-8 -*-
"""
Created on Wed Sep 25 10:36:31 2019
...
import pandas as pd
import numpy
df = pd.read_csv("Z:/users/jbackes/cc_cvi_data_ravi_DI.csv")
import matplotlib.pyplot as plt

plt.style.use('seaborn-deep')
bins = numpy.linspace(-50, 400, 50)
pyplot.hist(df['cvi_total_count'], bins, alpha=0.5, label='x')
pyplot.hist(df['no_cvi_total_count'], bins, alpha=0.5, label='y')
pyplot.hist(df['actual_count'], bins, alpha=0.5, label='z')
pyplot.legend(loc='upper right')
pyplot.savefig("Z:/users/jbackes/plot.pdf")
pyplot.show()

import math
odds = input("please select odd of happending a) 1 in million, b) 1 in 300 million (powerball odds), c) 1 in a billion, or d) 1 in a trillion -> ")
items = int(input("please type in the number of items, people, ideas, subjects, etc you typically encounter on a daily basis -> "))
...
if odds == 'c':
    prob = 1/1000000000
if odds == 'd':
    prob = 1/1000000000000
probablity = prob*(math.factorial(items)/math.factorial(items-2))/2
print(prob)
print(probablity)             
...
math.factorial(items)/math.factorial(items-2)/2
k = 0 
d = []
for i in range(2,5000):
    for j in range(1):
        k += 1
        print(1/300000000*(math.factorial(i)/math.factorial(i-2))/2)
...
# -*- coding: utf-8 -*-
"""
Created on Wed Dec 12 17:19:46 2018
...
VB1TJWHs2Lvamj3HwHK0

plotly.tools.set_credentials_file(username='jaysonbackes', api_key='VB1TJWHs2Lvamj3HwHK0')
py.sign_in('jaysonbackes','VB1TJWHs2Lvamj3HwHK0')
...
url = 'P:/Personal Projects/gapminderDataFiveYear.csv'
